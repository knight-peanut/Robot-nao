...
class BallDetect(VisualBasis):
	...
	def __init__(self, IP, PORT=9559, cameraId=vd.kBottomCamera, resolution=vd.kVGA,
		...
		#self.MotionBasis = MotionBasis(IP)
		self.speAngle=0
	...
	def __getChannelAndBlur(self, color,Hm=8):
		"""
	...
	def __findCircles(self, img, minDist, minRadius, maxRadius):
		...
		if cv_version == "3": # for OpenCV >= 3.0.0
			...
			gradient_name = cv2.CV_HOUGH_GRADIENT
		'''
	...
	def updateBallData(self, client="python_client", standState="standInit", color="red",
					   ...
					   maxHSV2=np.array([180,255,255]), saveFrameBin=False,Hm=8):
		"""
		...
		if colorSpace == "BGR":
			grayFrame = self.__getChannelAndBlur(color,Hm)
		else:
	...
	def RisingHeadSearching(self):
                for i in range(1):
			name =['HeadPitch','HeadYaw']
			targetangle = [-13.7*almath.TO_RAD,1.0*almath.TO_RAD]
			fractionMaxSpeed = 0.05
			self.motionProxy.angleInterpolationWithSpeed(name,targetangle,fractionMaxSpeed)
			self.updateBallData(client='xxx',fitting=True)
			x,y,angle=self.getBallPosition()
			if not x==0 and not y==0 and not angle==0:
				break

			names  = ["HeadYaw"]
			targetangles=40.3*almath.TO_RAD
			...
			targetangles=1.0*almath.TO_RAD
			self.motionProxy.angleInterpolationWithSpeed(names,targetangles,maxSpeedFraction)
			time.sleep(1.0)
			targetangles=-51.2*almath.TO_RAD
			self.motionProxy.angleInterpolationWithSpeed(names,targetangles,maxSpeedFraction)
        ...
        def CrouchingHeadSearching(self):
		for i in range(1):
			name =['HeadPitch','HeadYaw']
			targetangle = [22*almath.TO_RAD,1.0*almath.TO_RAD]
			fractionMaxSpeed = 0.05
			self.motionProxy.angleInterpolationWithSpeed(name,targetangle,fractionMaxSpeed)
			self.updateBallData(client='xxx',fitting=True,Hm=6)
			x,y,angle=self.getBallPosition()
			if not x==0 and not y==0 and not angle==0:
				break
			...
			self.motionProxy.angleInterpolationWithSpeed(names,targetangles,maxSpeedFraction)
			time.sleep(1.0)
			targetangles=-45.2*almath.TO_RAD
			self.motionProxy.angleInterpolationWithSpeed(names,targetangles,maxSpeedFraction)
	...
	def SearchingRedBall(self):
                self.RisingHeadSearching()
		x,y,angle=self.getBallPosition()
		self.showBallPosition()
		cv2.waitKey(0)
		#self.postureProxy.goToPosture("StandInit", 0.2)
		self.DefStandInit()		
                while x>0.4 or y>0.4 or y<-0.4 or angle>10*math.pi/180 or angle<-10*math.pi/180:
			#self.motionProxy.moveTo(x/2,0,angle,[['MaxStepFrequency',0.5],['StepHeight',0.01]])
                        if x>1.0 or y>1.0 or y<-1.0:
                                self.motionProxy.moveTo(0,0,angle/2,self.moveConfig)
                                self.motionProxy.moveTo(x/2,0,0,self.moveConfig)
                                time.sleep(2.0)
                        else:
		...
		if x==0 and y==0 and angle==0:
			...
			if x==0 and y==0 and angle==0:
                                self.DefStandInit()
                                #self.postureProxy.goToPosture("StandInit", 0.2)
                                #self.motionProxy.moveTo(x,0,angle,[['MaxStepFrequency',0.5],['StepHeight',0.01]])
                                self.motionProxy.moveTo(0,0,angle,self.moveConfig)
                                ...
                                self.DefStandInit()
                                #self.postureProxy.goToPosture("StandInit", 0.2)
                                #self.motionProxy.moveTo(x-0.3,0,angle,[['MaxStepFrequency',0.5],['StepHeight',0.01]])
                                self.motionProxy.moveTo(0,0,angle,self.moveConfig)
                                ...
                                self.showBallPosition()
                                cv2.waitKey(0)
                                while x>=0.45 and x<=0.2 and y>=0.05  and y<=-0.05 and angle>=0.6 and angle<=-0.6:
                                        self.DefStandInit()
                                        #self.postureProxy.goToPosture("StandInit", 0.2)
                                        #self.motionProxy.moveTo(x-0.3,0,angle,[['MaxStepFrequency',0.5],['StepHeight',0.01]])
                                        self.motionProxy.moveTo(0,0,angle,self.moveConfig)
	...
	def DefStandInit(self):
        ...
        def SearchingRedBall_task3(self):
                self.RisingHeadSearching()
                x,y,angle=self.getBallPosition()
                self.showBallPosition()
                #cv2.waitKey(0)
                ...
                while x>0.4 or y>0.4 or y<-0.4 or angle>10*math.pi/180 or angle<-10*math.pi/180:
                        ...
                        if x>1.0 or y>1.0 or y<-1.0:
                                self.motionProxy.moveTo(x/2,0,0,self.moveConfig)
                                #self.motionProxy.moveTo(0,y/2,0,[['MaxStepFrequency',0.5],['StepHeight',0.01]])
                                time.sleep(1.0)
                        else:
        ...
        def NewSearchingRedBall_task3(self):
                self.RisingHeadSearching()
                x,y,angle=self.getBallPosition()
                self.showBallPosition()
                #cv2.waitKey(0)
        ...
        def getspeAngle(self):
                if(self.speAngle>40*almath.TO_RAD):
                        self.speAngle=40*almath.TO_RAD
                if(self.speAngle<-40*almath.TO_RAD):
                        self.speAngle=-40*almath.TO_RAD
                return self.speAngle

        def RedballIsInSight(self):
                if self.ballPosition['disX']==0 and self.ballPosition['disY']==0 and self.ballPosition['angle']==0:
                        return False
                else:
                        return True
...
class LandMarkDetect(ConfigureNao):
	...
	def updateLandMarkData(self, client="landMark"):
		...
		self.SearchingLandMark()
		
		if (self.landmarkFlag==1):
			self.disX = 0
			self.disY = 0
			self.dist = 0
			self.yawAngle = 0
		else:
			wzCamera = self.allmarkdata[0]
			wyCamera = self.allmarkdata[1]
			angularSize = self.allmarkdata[2]
	...
	def SearchingLandMark(self):
                ...
                while (headYawAngle <= 2 ):
                    ...
                    if(markData and isinstance(markData, list) and len(markData) >= 2):
                        ...
                        headangle = self.motionProxy.getAngles("HeadYaw",True);
                        #print(headangle)                               
                        markheadangle = markwzCamera + headangle[0]
                        self.allmarkdata = [markwzCamera,markwyCamera,markangularSize,markheadangle,self.landmarkFlag]
                        #print(self.allmarkdata)
                        return
                        
                        break
                    else:
                        self.tts.say("where is landmark ?")
                        markwzCamera = 0
                        markwyCamera = 0
                        markangularSize = 0
                    
                    headYawAngle = headYawAngle + 0.8
                self.tts.say("i can not find landmark ! I will hit the ball directly ! ")
                print( "landmark is not in sight !")
                self.landmarkFlag = 1    
                self.allmarkdata = [0,0,0,0,self.landmarkFlag]
                
                self.landMarkProxy.unsubscribe("landmarkTest")

        def landmarkIsInSight(self):
		if(self.disX==0 and self.disY==0 and self.dist==0 and self.yawAngle==0):
                        return False
                else:
                        return True
