...
golfPositionJointAnglesR4  = [1.03549, 0.314159, 1.66742, 0.998676, -0.980268, 0.12]
...
def HitBall():
    ...
    if (alpha<0):
        ...
        motionPrx.angleInterpolationWithSpeed(PositionJointNamesR,golfPositionJointAnglesR3,0.4)
        time.sleep(3)
        motionPrx.angleInterpolationWithSpeed(PositionJointNamesR,golfPositionJointAnglesR4,0.25)
        time.sleep(2)
        motionPrx.angleInterpolationWithSpeed(PositionJointNamesR,golfPositionJointAnglesR1,0.4)
    if (alpha>0):
        motionPrx.angleInterpolationWithSpeed(PositionJointNamesR,golfPositionJointAnglesR2,0.4)
...
def firstSearchredball():
    ...
    for i in range(0,3):
        ...
        if (ballDatatest==None):                             
            ...
            motionPrx.angleInterpolationWithSpeed("HeadYaw", 0.0, 0.5)
            motionPrx.setMoveArmsEnabled(False, False)
            #motionPrx.moveTo(0.3,0.0,-math.pi*15/180,moveConfig)   
    ...
    if (abs(wzCamera2 - wzCamera1)<=0.01 and abs(wzCamera3 - wzCamera2)>0.01):
        tts.say("redball is in the right !")  #右边
        redballFlag = 0
...
def Analyize():
  ...
  if circles is None:
    ...
    for i in circles[0,:]:
        ...
        cv2.circle(img,(i[0],i[1]),i[2],(0,255,0),2)  #BGR
...
motionPrx.moveTo(0,0.0,8*math.pi/180,moveConfig)
...
########turndata=[-0.15,-0.13,math.pi*85/180,0]     
turndata[1]+=-0.05                                            
turndata[0]+=0.03
